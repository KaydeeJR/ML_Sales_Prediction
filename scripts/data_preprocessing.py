...
2) handling NaN values using SimpleImputer and InterpolationImputer
...
numeric_transformer = Pipeline(steps=[('imputer', InterpolationImputer(
    strategy='linear')), ('scaler', StandardScaler())])
...
class DFExtractor():
    ...
    def find_holidays(self):
        ...
        for y in range(len(years_list)):
            # k = dates, v = holiday name
            for k, v in holidays.US(years=years_list[y]).items():
...
if __name__ == "__main__":
    ...
    pd.DataFrame(preprocessor.transform(
        extractor.get_DF()))  # <- returns dataframe
